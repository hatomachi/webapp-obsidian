...
- **特定列のみ編集許可（推奨）**:
  ...
  editable:
    ...
    - "## 📍 現在地"
  # またはインライン配列
  editable: [status, priority, done, "## 📍 現在地"]
  ```
  指定された列のみセルをクリックして直接編集できるようになり、指定されていない列は **読み取り専用（ロック）** となります（見出し列は `"## 見出し名"` または `"見出し名"` で指定可能）。
- **全列編集可能（未指定時のデフォルト動作）**:
  ```yaml
...
- **全列読み取り専用（閲覧ロック）**:
  ...
  ```
- ※ `file`（ファイルリンク）列はファイルパス自体のリンクのため常に読み取り専用です。
...
| **見出し本文セクション** | 列名: `"## 見出し名"`（フォルダモード専用） | 本文直下の1〜2行プレビュー。<br>チェックリスト `[1/3]` があれば進捗バッジ化 | タップで **複数行Markdownエディタ** を展開。Ctrl+Enter (Cmd+Enter) で即保存 ＆ Git自動コミット |
